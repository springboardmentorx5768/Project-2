...
import ReactionButtons from './ReactionButtons'; // Yeh component zaroori hai

function ShoutoutFeed() {
    const [shoutouts, setShoutouts] = useState([]);
    const [commentInputs, setCommentInputs] = useState({});

    // --- WEEK 4: FILTER STATES ---
    ...
    // Fetch Shoutouts function (ab filters ke sath)
    const fetchShoutouts = useCallback(async () => {
        const token = getToken();
        if (!token) return;

        try {
            // Filters ko URL parameters mein add karein
            ...
            // Backend `crud.py` [cite: 512] pehle se hi in parameters ko support karta hai
            const response = await axios.get(`http://localhost:8000/shoutouts/`, {
                headers: { 'Authorization': `Bearer ${token}` },
                params: params // params ko yahan pass karein
            });
            setShoutouts(response.data);
        } catch (error) {
            console.error('Error fetching shoutouts:', error);
        }
    }, [department, senderEmail]); // Ab ye department ya email change hone par re-fetch karega

    useEffect(() => {
        fetchShoutouts();
    }, [fetchShoutouts]);

    // Filter clear karne ka handler
    const handleFilterClear = () => {
        setDepartment('');
        setSenderEmail('');
    };

    // --- BAAKI SAB LOGIC (REACTION/COMMENT) ---
    ...
    const handleCommentSubmit = async (shoutoutId) => {
        ...
        const token = getToken();
        try {
            // Backend `main.py` [cite: 704] comment endpoint
            ...
            fetchShoutouts(); // Naya comment aane par feed refresh karo
        } catch (error) {
            console.error('Failed to post comment', error);
        }
    };

    // Backend `main.py` [cite: 706] reaction endpoint
    ...
    const handleReact = async (shoutoutId, reactionType) => {
        // Optimistic UI update (turant response ke liye)
        ...
        // API call ko background mein bhej do
                ...
                {shoutouts.length > 0 ? shoutouts.map((shoutout) => (
                    <Card key={shoutout.id} sx={{ overflow: 'visible' }}>
                        <CardHeader
                            ...
                            subheader={`Tagged: ${shoutout.tagged_users}`}
                        />
                        ...
                        <CardContent>
                            ...
                            <List dense sx={{ width: '100%', p: 0 }}>
                                {shoutout.comments.map((comment) => (
                                    <ListItem key={comment.id} sx={{ alignItems: 'flex-start' }}>
                                        ...
                                        <ListItemText
                                            primary={<Typography variant="body2" component="span" fontWeight="600" sx={{ mr: 0.5 }}>{comment.author?.email || 'Unknown'}</Typography>}
                                            secondary={<Typography variant="body2" component="span" color="text.secondary">{comment.text}</Typography>}
                                        />
                                    </ListItem>
                                ))}
                            </List>
                            ...
                            <Box component="form" onSubmit={(e) => { e.preventDefault(); handleCommentSubmit(shoutout.id); }} sx={{ display: 'flex', mt: 2, gap: 1 }}>
                                ...
                                <TextField 
                                    ...
                                    sx={{ borderRadius: '20px', '& .MMuiOutlinedInput-root': { borderRadius: '20px' } }}
                                />
                    ...
                    // Agar filter ki vajah se kuch na mile
