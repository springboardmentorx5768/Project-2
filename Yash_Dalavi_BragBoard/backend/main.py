...
# --- UPDATED read_shoutouts ENDPOINT ---
@app.get("/shoutouts/", response_model=List[schemas.Shoutout])
def read_shoutouts(
    skip: int = 0, limit: int = 100,
    department: Optional[str] = None, # <-- ADDED
    sender_email: Optional[str] = None, # <-- ADDED
    current_user: schemas.User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    shoutouts = crud.get_shoutouts(db=db, skip=skip, limit=limit, department=department, sender_email=sender_email)
    return shoutouts
