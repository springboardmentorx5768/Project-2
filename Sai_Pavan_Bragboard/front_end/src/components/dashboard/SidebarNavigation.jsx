...
function SidebarNavigation() {
  ...
  return (
    <aside className="sidebar">
      ...
      <button className="create-shoutout-btn" onClick={handleCreate} aria-label="Create a new post">
        + Create Post
      </button>

      <nav className="sidebar-nav">
  <NavLink to="/dashboard" className={({ isActive }) => isActive ? "nav-link active" : "nav-link"} aria-label="Feed">
  ...
  <NavLink to="/my-shoutouts" className={({ isActive }) => isActive ? "nav-link active" : "nav-link"} aria-label="My Posts">
  ...
  <NavLink to="/department-feed" className={({ isActive }) => isActive ? "nav-link active" : "nav-link"} aria-label="Department">
  ...
  <NavLink to="/leaderboard" className={({ isActive }) => isActive ? "nav-link active" : "nav-link"} aria-label="Rankings">
  ...
  <NavLink to="/recognition-history" className={({ isActive }) => isActive ? "nav-link active" : "nav-link"} aria-label="Recognition">
          <svg className="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8l3.09 6.26L22 15l-5 4.87L18.18 23 12 19.77 5.82 23 7 19.87 2 15l6.91-0.74L12 8z" /></svg>
          Recognition
        </NavLink>

        {/* Admin link: visible to is_superuser or main admin email */}
        {user && (user.is_superuser || user.email === MAIN_ADMIN_EMAIL) && (
          <NavLink to="/admin" className={({ isActive }) => isActive ? "nav-link active" : "nav-link"} aria-label="Admin">
            <svg className="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 12c2.21 0 4-1.79 4-4S14.21 4 12 4 8 5.79 8 8s1.79 4 4 4zm0 2c-3.31 0-6 2.69-6 6h12c0-3.31-2.69-6-6-6z" /></svg>
            Admin
          </NavLink>
        )}

  <NavLink to="/profile" className={({ isActive }) => `nav-link bottom-section ${isActive ? 'active' : ''}`} aria-label="Profile">
  ...
  <NavLink to="/settings" className={({ isActive }) => isActive ? "nav-link active" : "nav-link"} aria-label="Settings">
