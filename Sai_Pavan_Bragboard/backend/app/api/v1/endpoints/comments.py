...
def create_comment(
    ...
    created = crud.crud_comment.create_comment(
        db=db,
        comment=comment,
        shoutout_id=shoutout_id,
        user_id=current_user.id
    )
    # Award points to commenter and to shoutout owner for receiving a comment
    try:
        shoutout_owner_id = shoutout.sender_id if shoutout else None
        crud_score.apply_comment_posted(db, commenter_id=current_user.id, shoutout_owner_id=shoutout_owner_id)
    except Exception:
        pass
    return created
