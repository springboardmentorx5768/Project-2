...
async def register_user(user: schemas.UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    if user.role == "admin":
        if not user.security_key or not user.security_key.strip():
            raise HTTPException(status_code=403, detail="Security key is required for admin registration")
        
        # Trim and normalize the security key
        key_trimmed = user.security_key.strip()
        
        # Check if key exists and is unused
        q = select(SecurityKey).where(
            SecurityKey.key == key_trimmed,
            SecurityKey.is_used == False
        )
        ...
        if not key_obj:
            ...
            q_used = select(SecurityKey).where(SecurityKey.key == key_trimmed)
            res_used = await db.execute(q_used)
            key_used = res_used.scalars().first()
            if key_used:
                raise HTTPException(status_code=403, detail="This security key has already been used")
            else:
                raise HTTPException(status_code=403, detail="Invalid security key. Please check and try again")
...
async def update_profile(
    ...
    if user_update.username and user_update.username != user.username:
        existing_username = await crud.get_user_by_username(db, user_update.username)
    ...
    if user_update.current_project is not None:
        user.current_project = user_update.current_project

    if user_update.group_members is not None:
        user.group_members = user_update.group_members

    # Password change
    if user_update.new_password:
        if not user_update.current_password:
            raise HTTPException(status_code=400, detail="Current password is required to set a new password")
        if not auth.verify_password(user_update.current_password, user.password):
